...
export class NoteCreateService implements OnApplicationShutdown {
	...
	public async fetchAndCreate(user: {
		...
		if (data.deleteAt != null) {
			if (data.deleteAt.getTime() < Date.now()) {
				throw new IdentifiableError('4ef48daf-141c-40f1-83c5-20d61695d427', 'Scheduled delete time is earlier than now');
			}

			if (data.deleteAt.getTime() > Date.now() + 365 * 24 * 60 * 60 * 1000) {
				throw new IdentifiableError('a1376a20-16e7-4151-af13-0b005b471d71', 'Scheduled delete time is later than one year');
			}
		}
