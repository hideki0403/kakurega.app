...
<SearchMarker path="/settings/preferences" :label="i18n.ts.preferences" :keywords="['general', 'preferences']" icon="ti ti-adjustments">
	<div class="_gaps_m">
							...
							<MkPreferenceContainer k="serverDisconnectedBehavior">
								<MkSelect
									...
									:items="[
										...
										{ label: i18n.ts.doNothing, value: 'none' },
									]"
								>
		...
		<div class="_gaps_s">
			...
			<FormLink to="/settings/zen"><template #icon><i class="ti ti-seeding"></i></template>{{ i18n.ts.zenMode }}</FormLink>
...
async function addItemPostform() {
	...
	postFormItems.value = [...postFormItems.value, {
		id: Math.random().toString(),
		type: item,
	}];
}
...
function removeItemPostform(type: keyof typeof postformBottomItemDef, ev: MouseEvent) {
	const item = postformBottomItemDef[type];
	os.popupMenu([{
		type: 'label',
		text: item.title,
	}, {
		text: i18n.ts.remove,
		action: () => {
			postFormItems.value = postFormItems.value.filter(x => x.type !== type);
		},
	}], getHTMLElement(ev));
}

async function savePostform() {
	prefer.commit('postFormActions', postFormItems.value.map(x => x.type));
}

async function resetPostform() {
	...
	postFormItems.value = PREF_DEF.postFormActions.default.map(x => ({
		id: Math.random().toString(),
		type: x,
	}));
}
